...
class Player:
    ...
    def get_champ(self, gotten_champ):
        ...
        if self.gold >= gotten_champ.stats['cost']:
            ...
            elif len([a for a in self.bench if a is None]) > 0:
    ...
    def put_champ(self, champ, cord):
        if self.on_grid < self.level:
            ...
            if cord[0] == -1 and loc[0] == 'grid':
                if len([a for a in self.bench if a is None]) > 0:
    ...
    def fight_ready(self):
        self.grid_check()
        if self.on_grid < self.level and len([a for a in self.bench if a is not None]) > 0:
        ...
        for x in range(10):
            ...
            while not game_over:
                ...
                if len(alive_players) % 2 == 1:
                    bot = copy(random.choice([bots for bots in alive_players if bots is not alive_players[len(alive_players) // 2]]))
                    bot.nick += '_bot'
                    initiate_fight(alive_players[len(alive_players) // 2], bot)
                    if not alive_players[len(alive_players) // 2].alive:
                        alive_players[len(alive_players) // 2].reward = 5 - len(alive_players)
                        send_back_to_pool(alive_players[len(alive_players) // 2].champions , pool)
                ...
                for pairs in range(int(len(alive_players) // 2)):
                    ...
                    if not alive_players[pairs].alive:
                        ...
                        send_back_to_pool(alive_players[pairs].champions , pool)
                    if not alive_players[-1 - pairs].alive:
                        alive_players[-1-pairs].reward = 5 - len(alive_players)
                        send_back_to_pool(alive_players[-1-pairs].champions , pool)
            ...
            for player in players:
                ...
                for x in range(len(player.history) - 1):
                    player.history[-2 - x]['Q'] = player.history[-2 - x]['Reward'] + player.history[-1]['Q']
